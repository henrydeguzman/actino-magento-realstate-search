...
(function($) {
    ...
    //console.log(classes);
                ...
                var retrieve = convert_time((Math.floor(label)/ 60), 0);
                if($.isNumeric(label)){
                    label = retrieve['hours'] + ':' + retrieve['minutes'];
                    if(label == "12:00 PM"){
                        label = "Noon";
                    }
                }
        ...
        float: function( settings ) {
            ...
            if ( options.handle ) {
                ...
                for( i = 0; i < tipValues.length; i++ ) {
                    console.log(tipValues[i]);
                    var hours1 = Math.floor(tipValues[i] / 60);
                    var minutes1 = tipValues[i] - (hours1 * 60);
                    var retrieve = convert_time(hours1, minutes1);
                    $handles
                        .eq( i )
                        .append( $("<span class=\"ui-slider-tip\">"+ retrieve['hours'] + ':' + retrieve['minutes'] +"</span>") );

                }
                //console.log(retrieve['hours'] + ':' + retrieve['minutes']);
            ...
            slider.element.on( options.event , function( e, ui ) {
                ...
                /*val = options.formatLabel( getPipLabels( uiValue )[0] );*/
                   // hours1 = options.formatLabel( getPipLabels(uiValue )[0]);

/*added*/


                var hours1 = Math.floor(getPipLabels(uiValue )[0] / 60);

                var minutes1 = getPipLabels(uiValue )[0] - (hours1 * 60);

                var retrieve = convert_time(hours1, minutes1);
               /* console.log("this" + retrieve["hours"]);
                console.log(retrieve['minutes']);*/
               // console.log(ui);
                ...
                $(ui.handle)
                    .find(".ui-slider-tip")
                    .html( retrieve["hours"] + ':' + retrieve["minutes"]);
